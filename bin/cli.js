...
const uploadFileToGithub = (content, fileType, { token, owner, repo, path }) =>
  octokit.request(`PUT https://api.github.com/repos/{owner}/{repo}/contents/{path}/${Date.now()}${index++}.${fileType}`, {
    headers: {
...
const uploadFileToGitte = (content, fileType, { token, owner, repo, path }) =>
  octokit.request(`POST https://gitee.com/api/v5/repos/{owner}/{repo}/contents/{path}/${Date.now()}${index++}.${fileType}`, {
    owner,
    repo,
    path,
    data: {
  ...
  .action(async (command) => {
    try {
      ...
      results.forEach(result => {
        console.log(result.data.content.download_url);
      })
